...
public function pc(Request $request) {
 ...
 \Config::set('app.debug', false);
     $validated = $request->validate([ 'purchase_code' => 'required' ]); 
     try { 
        $check= \Webmozart\Assert\Assert::test($request->purchase_code); 
        if ($check==true) {
         return redirect()->route('install.info'); 
        } else{ 
            $error=\Webmozart\Assert\Assert::$error; 
            return redirect('/install/purchase?msg='.$error); 
        } 
    } catch (Exception $e) { 
        return back(); 
    } 
