      ...
      <div class="container">
         <div class="row align-items-center">
            ...
            <div class="col-lg-6">
               ...
               <div class="text-lg-right top-right-bar mt-2 mt-lg-0">
                   @php
                    $currencies=currency_info_for_main();
                    $default = $currencies['currency_default']->currency_name;
                   @endphp
                   <div class="dropdown">
                       <button type="button" class="btn btn-success dropdown-toggle" data-toggle="dropdown">Currency</button>
                       <div class="dropdown-menu">
                           <a class="dropdown-item @if($default==Session::get('main_to_currency')) active @endif" href="{{url('/set_to_currency?'.'default='.$default.'&to='.$default)}}">{{$default.'  (default)'}}</a>
                           @foreach($currencies['additional_currencies'] as $key => $value)
                            <a class="dropdown-item @if($key == Session::get('main_to_currency')) active @endif" href="{{url('/set_to_currency?'.'default='.$default.'&to='.$key)}}">{{$key}}</a>
                           @endforeach
                       </div>
                   </div>
               </div>
