		...
		<div class="row">
			...
			<form action="{{ url('/payement/stripe') }}" method="post" id="payment-form">
				...
				<div class="form-group">
					...
					<button type="submit" class="btn btn-inline btn-block shadow-sm mt-2 checkout_submit_btn"> {{ __('Make Payment With Stripe') }} ({{ amount_format_order($amount,$session_data['currency']['icon']) }}) </button>
				</div>
			</form>
			<p>{{ $description }}</p>
			</div>
