...
@push('css')
<link rel="stylesheet" type="text/css" href="{{ asset('frontend/'.domain_info('user_id').'/bigbag/css/cart.css') }}">
@endpush
@section('content')  
@php
@endphp
                        ...
                        <div class="bigbag-cart-form">
                            <div class="table-responsive">
                                ...
                                <tr class="bigbag-cart-form__cart-item cart_item">
                                    ...
                                    <td><b>{{ amount_format_store($row->price) }}</b></td>
                                    <td class="text-center">{{ $row->qty }}</td>
                                    <td class="product-subtotal" data-title="Total">
                                        <span class="bigbag-Price-amount amount">{{ amount_format_store($row->price*$row->qty) }}</span></td>
                                </tr>
                                @endforeach
                                <tr>
                        ...
                        <table class="shop_table shop_table_responsive" cellspacing="0">
                            <tbody>
                                <tr class="cart-subtotal">
                                    ...
                                    <td data-title="Subtotal"><span class="bigbag-Price-amount amount">{{ amount_format_store(Cart::priceTotal()) }}</span></td>
                                </tr>
                                
                                
                                <tr class="cart-subtotal">
                                    <th>{{ __('Discount') }}</th>
                                    <td data-title="Subtotal"><span class="bigbag-Price-amount amount">- {{ amount_format_store(Cart::discount()) }}</span></td>
                                </tr>

                                <tr class="cart-subtotal">
                                    <th>{{ __('Tax') }}</th>
                                    <td data-title="Subtotal"><span class="bigbag-Price-amount amount">{{ amount_format_store(Cart::tax()) }}</span></td>
                                </tr>
                                 <tr class="cart-subtotal">
                                    <th>{{ __('Subtotal') }}</th>
                                    <td data-title="Subtotal"><span class="bigbag-Price-amount amount">{{ amount_format_store(Cart::subtotal()) }}</span></td>
                                </tr>
                                <tr class="order-total">
                                    <th>{{ __('Total') }}</th>
                                    <td data-title="Total"><strong><span class="bigbag-Price-amount amount text-dark">{{ amount_format_store(Cart::total()) }}</span></strong>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
