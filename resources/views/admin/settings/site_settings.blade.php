  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
       ...
       <div class="form-group row mb-4">
          <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3" > {{ __('Default Currency Name') }}</label>
         <div class="col-sm-12 col-md-7">
           <select name="currency_default" class="form-control col-sm-12">
             <option value="{{$default_currency->currency_name}},{{$default_currency->currency_icon}}">{{$default_currency->currency_name.'  '.$default_currency->currency_icon}}</option>
           </select>
         </div>
       </div>
       
       <div class="form-group row mb-4">
           <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{__('Addtional Currency Name & Icon')}}</label>   
           <div class="col-sm-12 col-md-7">
               <select name="currency_name[]" class="form-control select2" multiple="" id="currency_name" placeholder="Select your currency" >
                  @foreach($all_currencies as $currency)
                    @if(isset($currency->currencySymbol))
                        <option value="{{$currency->id}},{{$currency->currencySymbol}}" @if(array_key_exists($currency->id, $additional_currencies)) selected @endif>
        ...
        <div class="form-group row mb-4">
            <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{__('Currency Position')}}</label>   
            <div class="col-sm-12 col-md-7">
                <select class="form-control" name="currency_position">
                    @if(isset($currency_info->currency_default->currency_position))
                        <option value="left" @if($currency_info->currency_default->currency_position  == 'left') selected @endif>Left</option>
                        <option value="right" @if($currency_info->currency_default->currency_position  == 'right') selected @endif>Right</option>
                    @else
