...
{		
	...
	if (Cache::has(domain_info('user_id').'currency_info')) {
		...
		$currency_name=$value['currency_default']->currency_name;
		$currency_icon=$value['currency_default']->currency_icon;
...
function amount_format_store($amount,$type="icon")
{	
   
	if(url('/') == env('APP_URL')){

	if (!Cache::has(domain_info('user_id').'currency_info_for_main')) {
		currency_info_for_main();
	}
	if (Cache::has(domain_info('user_id').'currency_info_for_main')) {
		$value = currency_info_for_main();
        $rate = Session::get('main_rate_base') ?? 1;
        $currency_name=Session::get('main_to_currency') ?? $value['currency_default']->currency_name;
		$currency_position=$value['currency_position'];
        $currency_icon = '';
  		if($currency_name == $value['currency_default']->currency_name){
  		    $currency_icon=$value['currency_default']->currency_icon;
  		}else {
  		    if(isset($value['additional_currencies']->$currency_name)){
      		    $currency_icon=$value['additional_currencies']->$currency_name;
  		    }
  		}

		$number=number_format($amount * $rate,2);
		if ($type == "icon") {
	   	...
	   	if (Cache::has(domain_info('user_id').'currency_info')) {
	   		$value = Cache::get(domain_info('user_id').'currency_info');
	   		$rate = Session::get('rate_base') ?? 1;
	   		$currency_position=$value['currency_position'];
	   		$currency_name=Session::get('to_currency') ?? $value['currency_default']->currency_name;
	   		$currency_icon = '';
	   		if($currency_name == $value['currency_default']->currency_name){
	   		    $currency_icon=$value['currency_default']->currency_icon;
	   		}else {
	   		    if(isset($value['currency_name']->$currency_name)){
	   		        $currency_icon=$value['currency_name']->$currency_name;   
	   		    }
	   		}
	...
	if (Cache::has($url)) {
		return $r=cache()->remember(domain_info('user_id').'currency_info', 500, function () {
			...
			if (!empty($currency)) {
				...
				$dta['currency_default']=$info->currency_default;

				return $dta;
			}
			else{
                $default=["currency_name" => env("DEFAULT_CURRENCY_NAME"), "currency_icon" => env("DEFAULT_CURRENCY_ICON")];
                $default = json_encode($default);
                $default = json_decode($default);
                
                $default_ = [env("DEFAULT_CURRENCY_NAME") => env("DEFAULT_CURRENCY_ICON")];
                $default_ = json_encode($default_);
                $default_ = json_decode($default_);
                
				$dta['currency_position']='left';
				$dta['currency_name']=$default_;
				$dta['currency_default']=$default;
				return $dta;
			}
		});
		...
		return $r=cache()->remember('currency_info', 300, function () {
			$currency=\App\Option::where('key','currency_info')->first();
			$currency=json_decode($currency->value);
			$dta['currency_position']=$currency->currency_position;
			$dta['currency_name']=$currency->currency_default->currency_name;
			$dta['currency_default']=$currency->currency_default;
			return $dta;
		});
	}
}
...
function admin_currency_info()
{
	$r=cache()->remember('admin_currency_info', 300, function () {
		...
		$dta['currency_position']=$currency->currency_position;
		$dta['currency_name']=$currency->currency_default->currency_name;
		$dta['currency_icon']=$currency->currency_default->currency_icon;
		return $dta;
	});
}	
...
/* created by nurs 2022-05-04 */
function user_template_path() 
{
    list($frontend, $theme) = explode(".", base_view());
    return $frontend.'.'.domain_info('user_id').'.'.$theme;
}

/* created by nurs 2022-05-05 */
function seller_id() 
{
    if (Auth::user()->role_id == 4) {
        $shop_owner = \App\Models\User::where('role_id', 3)->where('domain_id',Auth::user()->domain_id)->first();
...
function locale_content($data, $field) {
    $data = json_decode($data);
	if ($data->{$field}) {
		return $data->{$field}->{Session::get('locale')} ?? "";
	}
	return "";
}

function currency_info_for_main(){
    $url = Request::getHost();
    $url = str_replace('www.','', $url);
    
    if(Cache::has($url)){
        return $r=cache()->remember('currency_info_for_main', 500, function () {
        	$currency=\App\Option::where('key','currency_info')->first();
        	$currency=json_decode($currency->value);
        	$dta['currency_position']=$currency->currency_position;
        	$dta['additional_currencies']=$currency->currency_name;
        	$dta['currency_default']=$currency->currency_default;
        	return $dta;
        });    
    }
    else{
        return $r=cache()->remember('currency_info_for_main', 300, function () {
    	$currency=\App\Option::where('key','currency_info')->first();
    	$currency=json_decode($currency->value);
    	$dta['currency_position']=$currency->currency_position;
    	$dta['additional_currencies']=$currency->currency_name;
...
function amount_format_order($amount, $icon){
    $number = number_format($amount,2);
    $currency = \App\Option::where('key', 'currency_info')->first();
    $currency = json_decode($currency->value);
...
//function save_to_currency($cur){
//  Cache::put("to_currency", $cur);
//}

//function save_from_currency($cur){
// Cache:put("from_currency", $cur);
//}