...
class Stripe
{
    ...
    public function view(){
        if(Session::has('stripe_payment') && Session::has('customer_order_info')){
            ...
            $credentials['currency']['name']=Session::get('to_currency');
            $credentials['currency']['icon'] = '';
            if(Session::get('to_currency') == $currency_info['currency_default']->currency_name){
                $credentials['currency']['icon'] = $currency_info['currency_default']->currency_icon;
            }else {
                foreach($currency_info['currency_name'] as $key => $value){
                    if($credentials['currency']['name'] == $key){
                        $credentials['currency']['icon'] = $value;
                    }
                }
            }
            
            // $domain  = $_SERVER['HTTP_HOST'];
            // $seller = DB::table('domains')->where('domain', $domain)->first();
          ...
          return view(user_template_path().'.payment.stripe',compact('amount','publishable_key','description'));
        }
        abort(404);
    }
    ...
    public function status(Request $request) {
      ...
      if(Session::has('stripe_payment') && Session::has('customer_order_info') && Session::has('stripe_credentials')){
        ...
        $currency=$stripe_credentials['currency']['name'];
